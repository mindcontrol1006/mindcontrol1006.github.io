...
1. 튜토리얼  동굴  맵  제작
   1-1 플레이어는 캄캄한 동굴에서 시작하게되며, 곳곳에 있는 횃불을 보고 길을 따라간다.
   1-2 동굴은 일자 형태이며, 일부 시체나 뼈가 곳곳에 존재한다.
   1-3 동굴에는 몬스터, 보스 몬스터와 전투를 진행할 만한 공간이 필요하다.
   1-4 동굴에서 나갔을 때 광활한 초원이 나타나게 된다.
2. 안내  메시지  UI  제작
   2-1 튜토리얼 때 기본적인 조작 방법과 팁을 알려주는 안내 메시지가 화면 중앙 하단에 하얀색 글씨로 출력이 된다.
   2-2 안내 메시지가 출력될 때 요정이 속삭이는 듯한 소리를 출력한다.
3. W,A,S,D  이동  구현
   3-1 전진 : W, 뒤로 돌아 전진 : S, 좌측 이동 : A, 우측 이동 : D
   3-2 각 이동 방향키는 2개까지 동시 입력할 수 있으며 마지막 입력된 2개의 방향대로 이동한다
   3-3 예를 들어 W와 A를 누르면 좌측 대각선으로 직진하고, W와 S를 동시에 누르게 되면 움직이지 않는다.
4. 마우스  좌클릭  공격,  우클릭  스킬  공격,  스페이스  바  구르기(회피)  구현
   4-1 마우스 좌클릭은 서로 다른 형태의 베기 형태가 존재한다(내려 베기, 올려 베기, 횡 베기), 만약 이동 중에 공격하더라도 공격이 취소되지 않는다.
   4-2 우클릭을 사용하면 스킬 공격이 발동된다. 땅을 진동시킬 만큼 강력한 일격을 가한다(베기 형태), 이동 중에 사용하더라도 스킬이 발동되며 재사용 대기시간은 10초이다.
   4-3 스페이스 바를 누르면 이동하는 방향으로 구르기를 시전하며 순간적으로 데미지를 안 받게 된다. 이는 장애물에 막힐 시 이동 거리는 없거나 줄어들 수 있지만 사용할 수 있다.

### [튜토리얼  :  전투]

5. 몬스터(적)  구현
   5-1 몬스터는 `오브젝트 분해` 항목을 참조하여 각각 다른 종류의 몬스터가 존재한다.
   5-2 몬스터는 일반 버전과 강화 버전이 있으며, 강화 버전은 체력과 공격력 등의 기본 수치가 더 높고 특수한 스킬을 사용할 수 있다.
   5-3 몬스터는 플레이어와 거리가 가까워져서 인식되면 자동적으로 플레이어를 공격한다.
   5-4 몬스터는 각 스폰 구역이 존재하며 사망 이후 5분이 지나면 리스폰한다.
   5-5 몬스터의 이동과 공격, 사망 모션은 다를 수 있으나 앞서 기술한 기본적인 요소는 플레이어와 동일하다.

6. 보스  몬스터(적)  구현
   6-1 보스 몬스터는 `오브젝트 분해` 항목을 참조하여 각각 다른 종류의 보스 몬스터가 존재한다.
   6-2 보스 몬스터는 일반, 강화 몬스터보다 체력, 공격력 등의 기본 스텟이 더 높고 특수한 스킬을 사용할 수 있다.
   6-3 몬스터는 플레이어와 거리가 가까워져서 인식되면 자동적으로 플레이어를 공격한다.
   6-4 보스 몬스터의 이동과 공격, 사망 모션은 다를 수 있으나 앞서 기술한 기본적인 요소는 플레이어와 동일하다.
   6-5 튜토리얼 동굴에서 보스  몬스터가  도망가는  연출을 구현한다. 보스 몬스터는 체력이 25%일 때 전투를 중단하고, 보스 몬스터의 대사가 화면 중하단에 출력되며("신의 은총이 벌써 깨어날 줄이야"와 같은 플레이어의 존재를 두려워하여 도망가는 대사) 플레이어의 이동 속도에 3배에 달하는 속도로 도망가서 전투를 진행할 수 없는 연출을 한다.
   

### [메인  게임]

7. 배경 그래픽 모델링 및 레벨 디자인 진행(`오브젝트 분해` 항목 참조)
   7-1 하위 항목들은 주변에 초원을 포함하여 디자인된다.
   7-2 마을
   7-3 경비소
   7-4 농장
   7-5 교도소
...
9. NPC  상호작용 및 퀘스트 시스템 구현
   8-1 일정 범위 내의 가까운 NPC 클릭을 통한 대화 진행 구현
   8-2 대화는 버튼 클릭으로 진행된다.
   8-3 퀘스트는 스토리 진행을 위한 지역 이동과, 그곳에 있는 몬스터는 n마리(5~10) 잡는 것, 또는 n% 토벌(정해진 수치를 미공개)하는 것과 특수한 보스 몬스터를 처치하기 위한 길잡이 역할을 한다.
   8-4 퀘스트 보상은 마을 주민들의 신뢰를 얻어 평판이 쌓이지만, 직접적으로 플레이어가 볼 수 있는 수치는 없고, 스토리 진행 시 마을에 더 환대를 받기 시작하고, 주 핵심은 스토리 길잡이 역할이다.
   
10. 전투  시스템  구현
    10-1 플레이어가 몬스터와 전투를 하면서 사망에 대한 처리로 게임이 종료되는 것을 잘 처리하는지 확인
    10-2 몬스터가 사망한다면 정해진 리스폰 시간에 스폰 장소에 소환되는지 확인
    10-3 피격과 데미지 처리가 잘 이루어지는지 확인
    
11. 장비  아이템  착용  및  장비창 구현
    11-1 장비 아이템은 몬스터 처치 시 획득할 수 있고 안내 메시지와 함께 자동으로 착용된다.
    11-2 장비창은 `대표 이미지` UI를 참고하여 착용한 장비에 따라 바뀌어져 보인다.
    11-3 스토리 진행을 하면서 얻는 새로운 장비 착용에 따라 플레이어가 얻는 체력과 공격력 등의 수치가 증가한다.
    
12. 코인(전리품)  시스템  구현
    12-1 몬스터가 사망할 시 자동적으로 코인이 획득되며 `대표 이미지` UI를 참고하여 우측 하단에 출력된다.
    12-2 몬스터마다 떨어뜨리는 코인의 양이 다르며, 코인으로 체력 회복 물약을 구입할 수 있다.

13. 체력 회복 물약 구현 및 상점 시스템 구현
    13-1 체력 회복 물약은 Q 키를 누르면 사용할 수 있고, 체력이 50% 회복되며 재사용 대기시간은 2분이다.
    13-2 체력 회복 물약은 소모성이므로 코인을 통해 상점에서 구매하여 충전할 수 있다.
    13-3 9번 항목의 NPC 상호작용을 참고하여 마우스 클릭으로 구입을 할 수 있으며, 정해진 비용으로 한 번 클릭으로 최대 한 개씩 살 수 있다.
