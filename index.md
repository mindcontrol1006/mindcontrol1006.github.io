...
6. 보스  몬스터(적)  구현<br>
   ...
   6-2 보스 몬스터는 일반, 강화 몬스터보다 체력, 공격력 등의 기본 스텟이 더 높고 특수한 스킬을 사용할 수 있다.<br>
...
10. 전투  시스템  구현<br>
    10-1 플레이어가 몬스터와 전투를 하면서 사망에 대한 처리로 게임이 종료되는 것을 잘 처리하는지 확인<br>
    10-2 몬스터가 사망한다면 정해진 리스폰 시간에 스폰 장소(적절한 무리 구역)에 소환되는지 확인<br>
    10-3 피격과 데미지 처리가 잘 이루어지는지 확인<br>
    
11. 장비  아이템  착용  및  장비창 구현<br>
    11-1 장비 아이템은 `오브젝트 분해` 항목을 참고하여 몬스터 처치 시 일정 확률(2%)로 획득할 수 있고 안내 메시지와 함께 자동으로 착용된다.<br>
    11-2 장비창은 `대표 이미지` UI를 참고하여 착용한 장비에 따라 바뀌어져 보인다.<br>
    11-3 스토리 진행을 하면서 얻는 새로운 장비 착용에 따라 플레이어가 얻는 체력과 공격력 등의 수치가 증가한다.<br>
    
12. 코인(전리품)  시스템  구현<br>
    12-1 몬스터가 사망할 시 자동적으로 코인이 획득되며 `대표 이미지` UI를 참고하여 우측 하단에 출력된다.<br>
    12-2 몬스터마다 떨어뜨리는 코인의 양이 다르며, 코인으로 체력 회복 물약을 구입할 수 있다.<br>

13. 체력 회복 물약 구현 및 상점 시스템 구현<br>
    13-1 체력 회복 물약은 Q 키를 누르면 사용할 수 있고, 체력이 50% 회복되며 재사용 대기시간은 2분이다.<br>
    13-2 체력 회복 물약은 소모성이므로 코인을 통해 상점에서 구매하여 충전할 수 있다.<br>
    13-3 9번 항목의 NPC 상호작용을 참고하여 마우스 클릭으로 구입을 할 수 있으며, 정해진 비용으로 한 번 클릭으로 최대 한 개씩 살 수 있다.<br>
