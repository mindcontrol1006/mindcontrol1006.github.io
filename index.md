...
4. 마우스  좌클릭  공격,  우클릭  스킬  공격,  스페이스  바  구르기(회피)  구현<br>
   4-1 마우스 좌클릭은 서로 다른 형태의 베기 형태가 존재한다(내려 베기, 올려 베기, 횡 베기), 만약 이동 중에 공격하더라도 공격이 취소되지 않는다.<br>
   4-2 우클릭을 사용하면 스킬 공격이 발동된다. 땅을 진동시킬 만큼 강력한 일격을 가한다(베기 형태), 이동 중에 사용하더라도 스킬이 발동되며 재사용 대기시간은 10초이다.<br>
   4-3 스페이스 바를 누르면 이동하는 방향으로 구르기를 시전하며 순간적으로 데미지를 안 받게 된다. 이는 장애물에 막힐 시 이동 거리는 없거나 줄어들 수 있지만 사용할 수 있다.<br>
...
12. 체력 회복 물약 구현 및 상점 시스템 구현<br>
  12-1 체력 회복 물약은 Q 키를 누르면 사용할 수 있고, 체력이 50% 회복되며 재사용 대기시간은 2분이다.<br>
...
1. 튜토리얼 동굴 맵 제작 (요구사항 1번)<br>
    1-1 플레이어는 캄캄한 동굴에서 시작하게되며, 곳곳에 있는 횃불을 보고 길을 따라간다.<br>
    1-2 동굴은 일자 형태이며, 일부 시체나 뼈가 곳곳에 존재한다.<br>
    1-3 동굴에는 몬스터, 보스 몬스터와 전투를 진행할 만한 공간이 필요하다.<br>

2. W,A,S,D 이동 구현 (요구사항 3번)<br>
    2-1 전진 : W, 뒤로 돌아 전진 : S, 좌측 이동 : A, 우측 이동 : D<br>
    2-2 각 이동 방향키는 2개까지 동시 입력할 수 있으며 마지막 입력된 2개의 방향대로 이동한다.<br>
    2-3 예를 들어 W와 A를 누르면 좌측 대각선으로 직진하고, W와 S를 동시에 누르게 되면 움직이지 않는다.<br>


3. 마우스 좌클릭 공격, 우클릭 스킬 공격, 스페이스 바 구르기(회피) 구현 (요구사항 4번)<br>
    3-1 마우스 좌클릭은 서로 다른 형태의 베기 형태가 존재한다(내려 베기, 올려 베기, 횡 베기), 만약 이동 중에 공격하더라도 공격이 취소되지 않는다.<br>
    3-2 우클릭을 사용하면 스킬 공격이 발동된다. 땅을 진동시킬 만큼 강력한 일격을 가한다(베기 형태), 이동 중에 사용하더라도 스킬이 발동되며 재사용 대기시간은 10초이다.<br>
    3-3 스페이스 바를 누르면 이동하는 방향으로 구르기를 시전하며 순간적으로 데미지를 안 받게 된다. 이는 장애물에 막힐 시 이동 거리는 없거나 줄어들 수 있지만 사용할 수 있다.<br>

4. 몬스터(적) 구현 (요구사항 5번)<br>
    4-1 몬스터는 오브젝트 분해 항목을 참조하여 각각 다른 종류의 몬스터가 존재한다.<br>
    4-2 몬스터는 플레이어와 거리가 가까워져서 인식되면 자동적으로 플레이어를 공격한다.<br>
    4-3 몬스터는 각 스폰 구역이 존재하며 사망 이후 5분이 지나면 리스폰한다.<br>
    4-4 몬스터의 이동과 공격, 사망 모션은 다를 수 있으나 앞서 기술한 기본적인 요소는 플레이어와 동일하다.<br>

5. 배경 그래픽 모델링 및 레벨 디자인 진행(오브젝트 분해 항목 참조) (요구사항 7번)<br>
    5-1 하위 항목들은 주변에 초원을 포함하여 디자인된다.<br>
    5-2 마을<br>
    5-3 경비소<br>

6. 전투 시스템 구현 (요구사항 9번)<br>
    6-1 플레이어가 몬스터와 전투를 하면서 사망에 대한 처리로 게임이 종료되는 것을 잘 처리하는지 확인<br>
    6-2 몬스터가 사망한다면 정해진 리스폰 시간에 스폰 장소에 소환되는지 확인<br>
    6-3 피격과 데미지 처리가 잘 이루어지는지 확인<br>
