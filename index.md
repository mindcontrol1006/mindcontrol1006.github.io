...
2. 마우스 좌클릭 공격, 우클릭 스킬 공격, 스페이스 바 구르기(회피) 구현 (요구사항 4번)<br>
    ~~2-1 마우스 좌클릭은 서로 다른 형태의 베기 형태가 존재한다(내려 베기, 올려 베기, 횡 베기), 만약 이동 중에 공격하더라도 공격이 취소되지 않는다.<br>~~
    2-2 우클릭을 사용하면 스킬 공격이 발동된다. 땅을 진동시킬 만큼 강력한 일격을 가한다(베기 형태), 이동 중에 사용하더라도 스킬이 발동되며 재사용 대기시간은 10초이다.<br>
    2-3 스페이스 바를 누르면 이동하는 방향으로 구르기를 시전하며 순간적으로 데미지를 안 받게 된다. 이는 장애물에 막힐 시 이동 거리는 없거나 줄어들 수 있지만 사용할 수 있다.<br>
